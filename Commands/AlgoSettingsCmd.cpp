...
void AlgoSettingsCmd::execute() {
    ...
    while (finished != 0) {
        if (!isNumber(token)) {
            dio->write("$print&string2$You must enter a number for K! Please enter new arguments:");
            ans = dio->read();
        ...
        if (classParams->setK(stoi(token)) != 1 && classParams->setMetric(ans) != 1) {
            dio->write("$print&string2$Both arguments are invalid! Please enter new arguments:");
            ans = dio->read();
        ...
        if (classParams->setK(stoi(token)) != 1) { // Setting the new value of K.
            dio->write("$print&string2$Invalid value of K, The new metric was set. Please enter new value for K:");
        ...
        if (classParams->setMetric(ans) != 1) { // Setting the new Metric.
            dio->write("$print&string2$Invalid metric, The new value of K was set. Please enter a new Metric:");
            ans = dio->read();
            continue;
        }
        finished = 0;
...
// A function that gets a string and checks if it is a number (if it contains only numbers).
bool AlgoSettingsCmd::isNumber(std::string s) {
    std::string::const_iterator it = s.begin();
    while (it != s.end() && std::isdigit(*it)) ++it; // Iterating over the characters in the string.
